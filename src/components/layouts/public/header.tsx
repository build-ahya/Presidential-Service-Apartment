...
export default function PublicHeader() {
  ...
  const { apartments } = useAppSelector((state) => state.apartment);
  ...
  const otherLinks = headerLinks.filter(
    (link) => !link.dropdown && !link.isButton
  );
    ...
    <header className='fixed top-4 left-1/2 z-50 w-full -translate-x-1/2 px-4'>
      <div className='mx-auto container'>
        <nav className='flex items-center justify-between'>
          ...
          <Link
            href='/'
            className='flex items-center gap-2 focus:outline-none border-0'
          >
            <Image
              src={
                isHome
          ...
          <div className='hidden md:flex items-center gap-2 px-4 py-3 rounded-full border border-white/20 bg-white/10 backdrop-blur supports-[backdrop-filter]:bg-white/10 shadow-lg'>
            ...
            {dropdownLinks.map((link) => (
              <div key={link.href} className='relative group'>
                ...
                {(link.label === 'Apartments'
                  ? Array.isArray(apartments) && apartments.length > 0
                  : Array.isArray(link.dropdown) && link.dropdown.length > 0) && (
                  <div className='absolute left-0 mt-0 hidden w-56 rounded-xl border border-white/20 bg-white/80 p-2 backdrop-blur shadow-md group-hover:block'>
                    {link.label === 'Apartments'
                      ? apartments.map((apt) => (
                          <Link
                            key={`${link.href}-${apt.slug}`}
              ...
              <DrawerContent className='p-4'>
                ...
                <div className='space-y-2'>
                  ...
                  <div className='pt-2 flex flex-wrap gap-2'>
                    {buttonLinks.filter((link) => link.label !== 'Search').map((link) => (
